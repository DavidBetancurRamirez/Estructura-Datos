...
class Famoso implements Comparable<Famoso>{
    ...
    public int compareTo(Famoso arg0) {
        int i = Integer.compare(cantidad_seguidores, arg0.getCantidad_seguidores());
        if (i!=0)
        	return i;
...
public class AVL_Famosos {
    ...
    public ArrayList<Famoso> mayoresSeguidores(int N, NodoB<Famoso> raiz) throws ExceptionNodo {
    	ArrayList<Famoso> famosos = new ArrayList<Famoso>();
    	famosos = mayoresSeguidores(N, raiz, famosos);
    	if (famosos.size()==0)
			throw new ExceptionNodo("No hay famosos con esa cantidad de seguidores");
    	return famosos;
    }
    private ArrayList<Famoso> mayoresSeguidores(int N, NodoB<Famoso> nodo, ArrayList<Famoso> arr) {
    	if (nodo!=null) {
    		mayoresSeguidores(N, nodo.getHijoDer(), arr);
	    	if (nodo.getLlave().getCantidad_seguidores()>=N) {
